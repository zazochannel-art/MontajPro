...
| **Calculator preț** | Alegi poziția, prețul și unitatea vin din tarifele din Setări; îți arată cât ai luat în medie pe unitate la lucrările trecute; deplasarea se socotește pe km, cu „dus-întors"; cantitățile pot veni direct dintr-o măsurătoare; salvează în lucrare sau ofertă |
...
| **Oferte trimise ca link** | Clientul deschide oferta în browser, fără cont, și o acceptă cu numele lui — confirmarea ajunge înapoi cu dată și oră; știi și când a deschis-o fără s-o accepte, poți trimite un memento, iar acceptarea se poate semna cu degetul |
...
| **Română și rusă** | Un comutator în Setări; ce n-are încă traducere apare în română, nu gol |
| **Banii statului** | Un procent pus o dată, iar Finanțele arată cât ai de pus deoparte pe lună și pe an |
| **Ruta zilei** | Opririle în ordinea orei, cu un buton care le deschide pe toate în hartă, plus avertizare când o zi e promisă de două ori |
| **Comandă la furnizor** | Lista de cumpărături grupată pe furnizor, trimisă pe WhatsApp ca o comandă |
| **Modificări neaplicate** | Când două dispozitive schimbă același lucru, versiunea care pierde nu dispare: se poate lua din Setări |
...
bară de semnal), `conflicts.test.ts` (rândul netrimis câștigă, dar versiunea
care pierde rămâne recuperabilă), `route.test.ts` (o avertizare falsă e mai rea
decât niciuna), `order.test.ts` (cantitatea trimisă la depozit e cantitatea
care se plătește) și `i18n.test.ts` (un text netradus rămâne în română).
...
backup → schimbarea monedei → comutarea în rusă → fișierele PWA.
...
src/
  ...
  lib/
    ...
    conflicts.ts      versiunile lăsate deoparte de sincronizare
    route.ts          ruta zilei și încărcarea ei
    order.ts          comanda la furnizor și mementoul către client
    i18n.ts           dicționarul ro→ru, cu româna drept cheie
